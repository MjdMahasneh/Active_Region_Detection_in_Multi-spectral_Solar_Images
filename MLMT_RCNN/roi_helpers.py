...
import pdb
import math
from . import MLMT_data_generators
import copy
...
def calc_iou(R, img_data, C, class_mapping, spect=None):
    ...
    for ix in range(R.shape[0]):
        ...
        for bbox_num in range(len(bboxes)):
            curr_iou = MLMT_data_generators.iou([gta[bbox_num, 0], gta[bbox_num, 2], gta[bbox_num, 1], gta[bbox_num, 3]],
                                                [x1, y1, x2, y2])
...
def non_max_suppression_fast_with_probs(boxes, probs, overlap_thresh=0.9, max_boxes=300):
